...
async function seedDatabase() {
  try {
    ...
    const { error: deleteUsersError } = await supabaseAdmin
      .from('users')
      .delete()
      .neq('id', '00000000-0000-0000-0000-000000000000'); // Safety to avoid deleting everything if condition fails
    ...
    if (deleteItemsError) {
      console.error('Error deleting scrapyard items:', deleteItemsError);
    } else {
      console.log('Scrapyard items table cleared');
    }
    ...
    for (const username in createdUsers) {
      ...
      // Update user with streetpass visitors
      await User.findByIdAndUpdate(user.id, {
        streetpassVisitors: streetpassVisitors
      });
