...
router.post('/item/:id/comment', ensureAuthenticated, async (req, res, next) => {
  try {
    const { text } = req.body;
    const itemId = req.params.id;
    ...
    if (!text || text.trim().length === 0) {
      ...
      return res.redirect(`/scrapyard/item/${itemId}`);
    }
    ...
    // Validate item ID format before querying
    if (!itemId.match(/^[0-9a-fA-F]{24}$/)) {
      req.flash('error_msg', 'Invalid item ID format');
    ...
    // Add the comment with proper sanitization
    const sanitizedText = text.trim().replace(/</g, '&lt;').replace(/>/g, '&gt;');

    item.comments.push({
      user: req.user._id,
      text: sanitizedText,
      createdAt: new Date() // Add timestamp for better tracking
    });
    ...
    try {
      ...
      console.warn('Failed to update user activity:', userErr);
    }

    req.flash('success_msg', 'Comment added successfully');
    res.redirect(`/scrapyard/item/${itemId}`);
  } catch (err) {
    console.error('Error adding comment:', err);

    // Use next() for consistent error handling
    next(new Error('Failed to add comment: ' + err.message));
  }
});
