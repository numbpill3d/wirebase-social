...
/**
 * Add digital noise for ambient effect
 * Optimized to use CSS will-change property for better performance
 */
function addDigitalNoise() {
    ...
    if (!document.querySelector('.digital-noise')) {
        ...
        noise.style.willChange = 'opacity'; // Optimize for animation
    ...
    if (!document.querySelector('.crt-scanlines')) {
        ...
        scanlines.style.willChange = 'opacity'; // Optimize for animation
    ...
    if (!document.querySelector('.crt-flicker')) {
        ...
        flicker.style.willChange = 'opacity'; // Optimize for animation
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.body.addEventListener('focus', (e) => {
    ...
    if (target.matches('a, button, input, select, textarea, [tabindex]:not([tabindex="-1"])')) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const initializeFeatures = () => {
        ...
        if (typeof setupCyberElements === 'function') {
            setupCyberElements();
        }

        if (typeof setupCyberEffects === 'function') {
            setupCyberEffects();
        }
        ...
        if (typeof requestIdleCallback === 'function') {
            requestIdleCallback(() => {
                if (typeof setupTerminalInterface === 'function') {
                    setupTerminalInterface();
                }
                if (typeof setupConnectionTracking === 'function') {
                    setupConnectionTracking();
                }
            });
        } else {
    ...
    // Don't call this here as it would duplicate initialization
    // The DOMContentLoaded event at the top of the file already initializes everything
});
