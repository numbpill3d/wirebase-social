...
function setupCyberEffects() {
    ...
    document.querySelectorAll('a:not([data-no-transition])').forEach(link => {
        if (link.href && link.href.indexOf(window.location.hostname) !== -1) {
            console.log('Adding click listener to link:', link.href, 'Text:', link.textContent);
            link.addEventListener('click', function(e) {
                ...
                try {
                    ...
                    if (!target || target !== '_blank') {
                        e.preventDefault();
                        console.log('Default prevented, playing sound...');
                        ...
                        console.log('Creating transition element...');
                        const transition = document.createElement('div');
                        transition.className = 'page-transition glitch-transition';
                        document.body.appendChild(transition);
                        console.log('Transition element created and added to DOM');
                        
                        console.log('Setting timeout for navigation...');
                        setTimeout(() => {
                            console.log('Timeout fired, navigating to:', this.href);
                            try {
                                window.location.href = this.href;
                            } catch (navErr) {
                                console.error('Error during navigation:', navErr);
                                // Try one more time with a direct assignment
                                window.location = this.href;
                            }
                        }, 300);
                    } else {
                        console.log('Link has target="_blank", using default behavior');
                    }
                } catch (err) {
                    console.error('Navigation error:', err);
                    // Fallback navigation in case of error
                    console.log('Attempting fallback navigation to:', this.href);
                    try {
                        window.location.href = this.href;
                    } catch (fallbackErr) {
                        console.error('Fallback navigation failed:', fallbackErr);
                        // Last resort - create and click a new anchor
                        const fallbackLink = document.createElement('a');
                        fallbackLink.href = this.href;
                        fallbackLink.setAttribute('data-no-transition', 'true');
