...
// Import directly from database.js instead of server.js to avoid circular dependency
...
console.log('knex object:', knex ? 'defined' : 'undefined');
console.log('knex.client:', knex?.client ? 'defined' : 'undefined');
...
const queryTimeoutMiddleware = (timeout = 30000) => {
  return (req, res, next) => {
    ...
    console.log('Middleware execution - knex:', knex ? 'defined' : 'undefined');
    console.log('Middleware execution - knex.client:', knex?.client ? 'defined' : 'undefined');
    
    // Check if knex and knex.client are defined before accessing
    if (!knex || !knex.client) {
      console.error('ERROR: knex or knex.client is undefined in query timeout middleware');
      return next();
    }
